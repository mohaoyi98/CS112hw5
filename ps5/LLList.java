 ...
 * 
 * This class implements the abstract data type of linked list.
 * 
 */
package ps5;
...
public class LLList implements List {
    ...
    public boolean removeAll(Object item) {
    	// if the item to be removed is null, 
    	// we automatically know that there is nothing to remove
    	if (item == null) {
    		return false;
    	}
    	
    	// the node previous to the traversal node
    	Node prev = this.head;
    	// the traversal node
    	...
    	// keep looping while there are still unchecked elements
    	while (trav != null) {
    		
    		// if a match is found
    		if (item.equals(trav.item)) {
    			// remove that matched node
    			prev.next = trav.next;
    			// update the length
    			this.length--;
    			// note that there is no need to change prev here
